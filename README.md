...
- **Refresh rate** (default 60Hz): The virtual display becomes the main display, and WindowServer coalesces pointer events to the main display's refresh rate, so a 60Hz virtual display caps cursor sampling at 60Hz on every screen including a 120Hz built-in panel. Selecting 120Hz under Refresh Rate in the menu bar lifts that cap while the hardware mirror drops frames to the 60Hz panel. The mirror path composites the full 7680x4320 surface every frame, so 120Hz roughly doubles WindowServer GPU load and keeps the GPU busy at idle (measured 6 to 44% idle and 24 to 26% moving windows at 120Hz, against 0% idle and 0 to 16% moving at 60Hz on an M5 Max). The menu asks for confirmation before switching to 120Hz.
